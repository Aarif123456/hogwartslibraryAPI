...
/* User is eligible for check out if they meet the following criteria 
    ...
    2. Are one of the following
        ...
        c. headmaster
*/
function checkUserEligbleForCheckout($userID, $conn){
    $memberActive = "SELECT memberID FROM members WHERE status=". ACCOUNT_ACTIVE;
    $slctStrt = " AND memberID IN (";
    $combiner = " UNION ";
    ...
    $stmt->bind_param("iii",$userID,     // Condition 2a
                            $userID,     // Condition 2b
                            $userID);    // Condition 2c
...
function createCheckOutTransaction($bookBarcode, $borrowedBy, $librarianID, $conn){
    ...
    $transactionStmt->bind_param("iiiiiiiiiiiii", 
    ...
    if(empty($tranId)) exit(CHECKOUT_FAILED);  
    return $tranId;
}
...
function updateCheckOutInfo($id, $conn){
    ...
    if($conn->affected_rows!=2) exit(CHECKOUT_UPDATE_FAILED);   
}
