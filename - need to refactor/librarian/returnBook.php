...
{  
  ...
  try{
    ...
    $lostStmt = $conn->prepare("SELECT lostDate,onHold FROM bookItem WHERE bookBarcode=?"); //check if book is issued to user
    ...
    if( !(empty($lost))){ //book was lost
        $foundStmt = $conn->prepare("UPDATE bookItem SET lostDate = NULL, onHold=1 WHERE bookBarcode =? AND lostDate IS NOT NULL");
    ...
    $returnStmt = $conn->prepare("UPDATE transactions SET returnDate = DATE(?), returnerID=? WHERE bookBarcode =? AND returnDate IS NULL");
