...
if (checkSessionInfo() && isset($_SESSION['userType'])) {  
  ...
  elseif(validateProfessor($_SESSION['userID']) && !($_SESSION['librarian'])) {
...
function addReservation($courseID,$bookISBN, $numCopies,$conn){
  $reserveStmt = $conn->prepare("UPDATE bookItem SET reservedFor = ? WHERE reservedFor IS NULL AND bookISBN = ? LIMIT ?"); 
