...
{  
    ...
    if(isValidPostVar('holdID')) {
        ...
        if(!$holdRslt || $holdRslt->num_rows ==0){
        ...
        if(!(empty($holdRslt['reservedCopy']))){
            $holdReserveStmt = $conn->prepare("UPDATE bookItem SET onHold = 0 WHERE bookBarcode =?");
            $holdReserveStmt->bind_param("i", $holdRslt['reservedCopy']);
            ...
            $holdUpdateStmt = $conn->prepare("UPDATE holds SET queueNumber = queueNumber -1 WHERE bookISBN =? AND queueNumber > ?");
            $holdUpdateStmt->bind_param("si",  $holdRslt['bookISBN'],$holdRslt['queueNumber']);
