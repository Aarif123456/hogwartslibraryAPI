...
if (validateUser()) {
    ...
    if (isset($_POST['bookBarcode']) && !(empty($_POST['bookBarcode']))) {
        ...
        $lostStmt = $conn->prepare(
            "UPDATE `bookItem` SET lostDate = DATE(?),onHold=1 WHERE bookBarcode =? and lostDate IS NULL"
        );
...
function checkBorrower($bookBarcode, $borrowerID, $conn)
{
    $borrowStmt = $conn->prepare(
        "SELECT * FROM transactions WHERE bookBarcode=? AND borrowedBy =? AND returnDate IS NULL"
