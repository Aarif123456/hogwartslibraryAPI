    ...
    if (isset($_POST['bookBarcode']) && !(empty($_POST['bookBarcode']))) {
        ...
        if ($renewedTime < 2) { //Can renew  a book Maximum of 2 times
            ...
            $renewStmt = $conn->prepare(
                "UPDATE `transactions` SET renewedTime = renewedTime +1 WHERE borrowedBy =? AND bookBarcode =? AND returnDate IS NULL"
            );
...
function checkBorrower($bookBarcode, $borrowerID, $conn)
{
    $borrowStmt = $conn->prepare(
        "SELECT renewedTime FROM `transactions` WHERE bookBarcode=? AND borrowedBy =? AND returnDate IS NULL"
...
function checkBookReservation($bookBarcode, $conn)
{
    $reservationStmt = $conn->prepare(
        "SELECT lostDate,reservedFor,onHold,holds,remainingCopies FROM `bookItem` NATURAL JOIN `books` WHERE bookBarcode=?"
    );
