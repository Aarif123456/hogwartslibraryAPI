...
{  
    ...
    elseif(validateUser($_POST['userID'])) {
    ...
    if(isValidPostVar('bookISBN')) {
        ...
        if(isset($_POST['courseID'])){
            ...
            $copyStmt = $conn->prepare("SELECT  COUNT(*) as cnt,bookBarcode FROM `bookItem` WHERE bookISBN = ? AND lostDate IS NULL AND onHold =0 AND checkedOutFlag = 0 AND reservedFor =? LIMIT 1");
        $copyStmt->bind_param("si", $bookISBN,$_POST['courseID']);
            ...
            $copyStmt = $conn->prepare("SELECT  COUNT(*) as cnt,bookBarcode FROM `bookItem `WHERE bookISBN = ? AND lostDate IS NULL AND onHold =0 AND checkedOutFlag = 0 AND reservedFor IS NULL LIMIT 1");
